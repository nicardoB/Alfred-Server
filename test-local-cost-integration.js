...
/**
 * Test cost tracking integration locally
 */

import { SmartAIRouter } from './src/ai/SmartAIRouter.js';
import { costTracker } from './src/monitoring/CostTracker.js';

async function testLocalCostIntegration() {
  console.log('🧪 Testing cost tracking integration locally...\n');
  ...
  try {
    ...
    console.log('Direct tracking result:', trackingResult);
    ...
    const aiResult = await router.processTextCommand('Hello, this is a test message.', {
      sessionId: 'test-session',
    ...
    if (requestChange >= 2 && tokenChange > 0 && costChange > 0) {
      console.log('✅ SUCCESS: Cost tracking integration is working locally!');
    } else if (requestChange >= 1) {
      console.log('⚠️  PARTIAL: Direct tracking works, but SmartAIRouter integration may have issues.');
    } else {
      console.log('❌ FAILED: Cost tracking not working locally.');
    }
